...
class onnus {
    ...
    static async initialize() {
        ...
        onnus.data.cache = onnus.data.cache ?? [];
        onnus.data.cache.body = onnus.data.cache.body ?? [];
        onnus.data.cache.tray = onnus.data.cache.tray ?? [];
        onnus.data.cache.interface = onnus.data.cache.interface ?? [];
        ...
        window.addEventListener('load', function(event) {
            ...
            // setTimeout(onnus.initialized, 500); // TEMP METHOD
    ...
    static async import(
        ...
        this.data.index = this.data.index ?? false;
        ...
        str_js_file_directory = '/' + this.data.index.directory['fra_res_js'];

        for (let int in arr_js_name) {

            str_js_name = arr_js_name[int];

            str_js_file_name = str_js_prefix + str_js_name;
            str_js_file_path = this.data.index['js'][str_js_file_name];
            ...
            if (
                ...
                let str_js_file =  str_js_file_directory + str_js_file_path;

                this.data['js'][str_js_file_name] = str_js_file;

                this.log('OJF 🔆 Status | onnus | import | 🟩 str_js_name = ' + str_js_name,'','js');
    ...
    static async cache(
        ...
        for (let str_cache_path in onnus.data.cache[str_cache]) {
            ...
            if (str_cache_path.includes('.css')) {
                ...
                delete this.data.css[str_cache_path];
                delete this.data.style[str_cache_path];
                ...
                delete this.data.js[str_cache_path];
                delete this.data.script[str_cache_path];
